...
public class VoicePlugin implements FlutterPlugin, MethodChannel.MethodCallHandler, EventChannel.StreamHandler,
    ...
    //    BackgroundCallJavaActivity backgroundCallJavaActivity= new BackgroundCallJavaActivity();
...
//    private AudioSwitch audioSwitch;
    ...
    private static void register(BinaryMessenger messenger, VoicePlugin plugin, Context context) {
    ...
    private static class VoiceBroadcastReceiver extends BroadcastReceiver {
        ...
        private VoiceBroadcastReceiver(VoicePlugin plugin) {
            this.plugin = plugin;
        }
    ...
    public void onMethodCall(@NonNull MethodCall call, @NonNull Result result) {
        ...
        } else if (call.method.equals("callerUserLogId")) {
            String caller = call.argument("callerUserLogId");
            Log.d(TAG, "Updating CallerFromTwilio " + caller);
            if (caller != null) {
                SharedPreferences.Editor edit = pSharedPref.edit();
                edit.putString("callerUserLogId", caller);
                edit.apply();
                edit.commit();
            }
        } else if (call.method.equals("callerToken")) {
            String caller = call.argument("callerToken");
            Log.d(TAG, "Updating CallerFromTwilio Token " + caller);
            if (caller != null) {

                SharedPreferences.Editor edit = pSharedPref.edit();
                edit.putString("callerToken", caller);
                edit.apply();
                edit.commit();

            }
        } else if (call.method.equals("callerDetails")) {
            String caller = call.argument("callerDetails");
            if (caller != null) {
                SharedPreferences.Editor edit = pSharedPref.edit();
                edit.putString("callerDetails", caller);
                edit.apply();
                edit.commit();
            }
...
//                backgroundCallJavaActivity.startAudioSwitch();
// setAudioFocus(false);
//                backgroundCallJavaActivity.audioSwitch.deactivate();
...
//                eventSink.success("LOG|Connected");
                activeCall = call;
//                backgroundCallJavaActivity.audioSwitch.activate();
                /*
